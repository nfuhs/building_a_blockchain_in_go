...
//CoinbaseTx is the function that will run when someone on a node succesfully "mines" a block.
//The reward inside as it were.
func CoinbaseTx(toAdress, data string) *Transaction {
	if data == "" {
		data = fmt.Sprintf("Coins to %s", toAdress)
	}
	...
	//txOut will represent the amount of tokens(reward) given to
	// the person(toAddress) that executed CoinbaseTx
